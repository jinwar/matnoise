...
%  A version of CalcPhaseV_Aki that limit the number of jumps in selecting a phase velocity dispersion 
%  that give a smoother Phase velocity dispersion
...
c_range=[0.7,0.7];
f0= 1/5;
maxjump=1; % maximum number of jump allowed between different dispersion curves c(T,m)
%% input argument
...
mtx_c(:,im)=0.5 * (c_up+c_dn); % average up and down crossings
...
for i=1:length(freqzero)
    ...
    switch n
        case 1 % if this is the first valid velocity to be determined
           ...
           if sum(condition)==1
			   m0=find(condition); % m value that c(n)=mtx_c(i,ind_m)
        ...
        case 2       
           ...
           if sum(condition)>=1
			   ...
			   ctemp=vec_c(find(condition3));
			   m1=find(mtx_c(i,:)==ctemp); % determine the value m			  
			   
			   			   
			   if(m1~=m0)
				   njump=njump+1;
				   m0=m1; % assign m1 to m0, for comparison next step
			   end

			   if(njump>=maxjump)
					display('maximum allowed jump exceeded, discard curve');
					break				
				end								
			   ...
			   njump=njump+1; % if no velocity is selected , consider it a jump
        ...
        otherwise
               ...
               if sum(condition)>=1 % if all the above conditions are met, choose a point that yield a smoothest curve
				   ...
				   if(m1~=m0)
					...
					if(njump>=maxjump)
						display('maximum allowed jump exceeded, finish extracting and save');
						break
					end
                   c(n)=ctemp;
                   freq(n)=freqzero(i);
...
plot(1./freqzero(1:i),vmax,'-.r','linewidth',0.5);
plot(1./freqzero(1:i),vmin,'-.r','linewidth',0.5);
