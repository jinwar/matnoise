function [resp dt]=calcinstresp_debug(dbsnin,sta,chan,time,timelength, lo_corner)
...
figure(1)
clf
semilogy(frq/2/pi,abs(resp),'x');
xlim([0 0.1])
figure(2)
clf
plot(frq/2/pi,angle(resp),'x');
xlim([0 0.1])
figure(3)
clf
plot(frq/2/pi,abs(hpfiltfrq),'x');
xlim([0 0.1])
figure(4)
clf
plot(frq/2/pi,angle(hpfiltfrq),'x');
xlim([0 0.1])
